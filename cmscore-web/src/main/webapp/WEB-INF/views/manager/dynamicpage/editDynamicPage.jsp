...
<form:form method="POST" action="/managercms/dynamiccontent/savedcontent.html" modelAttribute="dynamicContent">
    ...
    <div>
        ...
        <c:if test="${not empty shablonName}">
            <form:select id="shName" path="shablonName">
            </form:select>
        </c:if>
        <c:if test="${empty shablonName}">
            <select id="shName">
                <option value='root'>по умолчанию</option>
            </select>
        </c:if>
                ...
                loaddataconteiners:function(){
                    ...
                    $.ajax({
                        ...
                        for(var i=0;i<data.length;i++){
                            ...
                            var tmp1 = el.find('select[class=c_type] option[value='+data[i].type+']')[0];
